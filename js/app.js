...
jQuery(function ($) {
  ...
  var ContactFormApp = {
    ...
    validate: function () {
      ...
      //phone validation
      var phone_compare =
        /^(?:(?:\+|0{0,2})91(\s*[\ -]\s*)?|[0]?)?[6-9]\d{9}|(\d[ -]?){10}\d$/; // Syntax to compare against input
      var phone = this.$contactFormPhone.val(); // get the value of the input field

      if (phone == "" || phone == " " || phone == "Phone") {
        // check if the field is empty
        this.$contactFormPhone.fadeIn("slow"); // error - empty
        error = true;
      } else if (!phone_compare.test(phone)) {
        // if it's not empty check the format against our phone_compare variable
